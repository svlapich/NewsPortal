...
function drawPosts(postImage, postTitle, postText, postCategory) {
  ...
  card.innerHTML =
    ...
    '             <a id='+postCategory.replace(/\s/g, "_")+'_read-more class="mdl-button mdl-button--colored mdl-js-button mdl-js-ripple-effect">Read more</a\n' +
    '           </div>\n' +
    ' </div>\n';
  let content = document.getElementById("content");
  content.appendChild(card);
  let button = document.getElementById(postCategory.replace(/\s/g, "_") + "_read-more");
  button.addEventListener('click', function() { 
    getArticle(postImage, postTitle, postText), false });
}
...
function getArticle(postImage, postTitle, postText) {
  let OpenWindow = window.open("post.html");
